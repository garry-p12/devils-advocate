...
**Remaining adversarial misses** (3 of 17 FAIL at this iteration): three
cases each fire one structural feature but the trimmed mean sits at DAS ≈
0.78–0.79 — just under threshold. (Iteration 16's compound detectors and
iteration 17's Path A later recovered one of these each, settling the corpus
at **15/17 = 92.59%**; the current two misses — A5 and A14 — are detailed in
§7.3.) Documented in §7 rather than patched by adding the specific phrasings as
more tokens. The principled fix is a semantic scorer (future-work item #2).
...
### Iteration 17 — Round 3 reviewer fix: free-text structure extraction (Path A) + self-attack battery

The round-3 reviewer made a distinction the round-2 prose had blurred:
`structural.py` is not one mechanism but **three**, with different
generalisation properties.

1. **Evidence-dict field parsing** (`n`, design keys, `replicated_by`,
   surrogate/population key-pairs) — genuine structure parsing. Fires
   regardless of free-text phrasing.
2. **Free-text token lists** (`_STOPPED_EARLY_TOKENS`, `_NO_CONTROL_TOKENS`,
   `_WEAK_SOURCE_TOKENS`, `_SUBJECTIVE_OUTCOME_TOKENS`) — still substring
   lists. Better vocabulary than round 1, but they fail on the next paraphrase.
3. **The semantic ceiling** — weakness with no structural handle at all.

The reviewer's 8-case probe set (`heldout_datss_probes_v2.py`, dropped in
verbatim) made the split operational: **3/8** before this iteration. The 3
caught cases all had `n` in the evidence dict (DAS 0.917); the 5 free-text-only
misses sat at the ~0.77 base prior, untouched, because the tokens never matched.

**Path A — extract structure from free text** with regexes over a *class* of
phrasings instead of fixed substrings (`structural.py`):
...
- `_SINGLE_PERSON_RE` — a named single person ("a colleague", "my neighbour") ⇒
  `n=1`.
- `_INFORMAL_SOURCE_RE` — informal provenance as a shape (online forum, social
...
- `_NO_COMPARISON_RE` — explicit absence of a comparison ("nothing to compare
  against", "no group to compare it with").
- `_OUTCOME_STOP_RE` — outcome-dependent stopping (a termination verb whose
  trigger is the data turning favourable).

I also let `no_control` and `stopped_early` feed the evidence-quality,
...
whole problem. With those, v2 goes to **8/8**, threshold unchanged.

**Then the self-attack discipline**, because 8/8 on the reviewer's own probes is
exactly where overfitting hides:

- `path_a_generalization_check.py` — 10 independently-authored paraphrases of
  the covered classes (**10/10 caught**) plus 3 semantic-ceiling cases
  (**3/3 still missed**, by design). Evidence that Path A extracts structure
  rather than memorising the eight strings.
- `extra_probes.py` extended with **cat J** (5 fresh weak paraphrases, 5/5
  caught) and **cat K** (5 false-positive guards — strong claims phrased to
  *look* weak, 5/5 correctly PASS). Cat K surfaced a real overfit: bare "forum"
  matched expert/consensus forums (latent false positive, masked by strong
  evidence). Fixed to require an online-forum qualifier; the v2 forum probe
  still fires via "thread on".
- `self_attack.py` — nine diagnostics (vocabulary substitution, dict-vs-text
  isolation, cross-domain transfer, minimal perturbation, null-dict,
  PASS-stripping, paraphrase battery, compound-vs-single, threshold
  sensitivity). Two findings hardened the code: the forum precision fix above,
  and a **schedule-driven-stop guard** (`_SCHEDULED_STOP_TOKENS`) so a
  prespecified / group-sequential interim stop no longer reads as optional
  stopping (a precision gain — the cat-K case had passed only by luck of
  sentence length).

What I deliberately did **not** do: extend the verb/noun lexicons to pass my
own self-attack probes. Method 1 (vocabulary substitution) misses 5/5 and
Method 3 (cross-domain) 2/4 — these are the *demonstration* of the ceiling, not
bugs to patch. Adding the missing strings (a) only defers the miss one
paraphrase, exactly the round-1 mistake, and (b) provably trades precision:
broadening single-subject nouns to "single trial/run/sample" false-fires on
"a single large trial of 4,000". The honest fix is the semantic scorer
(§8b item 1), not a longer list.

Results after iteration 17:

- v2 reviewer probes (`heldout_datss_probes_v2.py`): **3/8 → 8/8**.
- Round-1 reviewer probes: unchanged at 10/10.
- `extra_probes.py` (now 36 cases): **18/25 → 29/35 = 82.86%**, 12/12 PASS.
- `path_a_generalization_check.py`: 10/10 caught, 3/3 ceiling missed.
- All 20 pytest tests pass; tuned threshold unchanged at 0.80.
...
### 4.6 Held-out probe sets (rounds 2 and 3)

The held-out evaluations live at repo root and in `datss/evaluation/`:
...
- **`heldout_datss_probes_v2.py`** — the round-3 reviewer's 8-case probe set,
  dropped in verbatim. Pre-iteration-17 result: **3/8** (only the cases with
  `n` in the evidence dict); post-Path-A result: **8/8**, threshold unchanged.
  Reproduces the reviewer's exact DAS values. See iteration 17 and §5.7.

- **`path_a_generalization_check.py`** — my own generalisation test for Path A:
  10 independently-authored paraphrases (10/10 caught) + 3 semantic-ceiling
  cases (3/3 still missed, by design). The honest check that Path A extracts
  structure rather than memorising the reviewer's eight strings.

- **`self_attack.py`** — the nine-method self-attack battery (§7.10), reporting
  raw behaviour including the documented vocabulary/cross-domain ceiling.

- **`adversarial_cases.csv`** + **`run_adversarial.py`** — the 30-case
  adversary-first corpus I authored after reading the round-2 letter.
  Result: **92.59%** on decisive cases (10/10 PASS, 15/17 FAIL). The 2
...
(§4.6) is the proper out-of-distribution check, and it returns 92.59%,
not 100%.
...
**Round-3 calibration (the part that matters as much as the code).** The
round-3 reviewer's sharpest point was not about a missed case — it was that the
prose ran a step ahead of the code. The round-2 claim "the structural tokens
generalise across domains" was true of layer 1 (evidence-dict parsing) and
overstated for layer 2 (the free-text token lists), and the gap was the same
shape as the round-1 YAML/code drift. Iteration 17 closes it on both ends: Path
A makes layer 2 genuinely generalise across a *class* of phrasings (verified at
10/10 on held-out paraphrases and guarded against over-firing by `extra_probes`
cat K), and the docs (README, this section, §7.8/§7.10) now state the exact
boundary — what each layer does, and where the vocabulary ceiling still bites.
The standard is: a reader trusting these docs should be able to predict what the
gate will and will not catch.
...
The current gate catches the round-1 reviewer's full 10-case probe set and
**25/27** of an adversary-first 30-case corpus (15/17 FAIL + 10/10 PASS,
**92.59%**; `adversarial_cases.csv`, canonical numbers in
`adversarial_report.json`). The **2** remaining FAIL misses share a profile:
one structural feature fires but the rest of the pool sits at the BASE_SCORE
prior, so the trimmed mean lands just under threshold.

- **A5** — "Once the early numbers from the trial trended positive we decided
  to wrap things up and write it up." (`n=18`). The optional-stopping/post-hoc
  language fires, but a single design flaw at n=18 only lifts the trimmed mean
  to **DAS 0.796** — one challenger-band short of 0.80.
- **A14** — "Patients on the new schedule appeared to do better in the eyes of
  the treating physicians." (unblinded assessment, no `n`). The
  clinical-impression/subjective signal fires alone; **DAS 0.774**.

(Earlier drafts of this section listed A2 and A6 here; iteration 16's compound
detectors and iteration 17's Path A now catch both — A2 at DAS 0.879, A6 at
0.857 — so the current misses are A5 and A14, verified against
`adversarial_report.json`.)
...
at 92.59%. Real deployment should expand to several hundred cases,
...
### 7.8 The free-text extractors are vocabulary-bound (the round-3 ceiling)

Iterations 14/16 made the scorer robust against the round-2 probe set, and
iteration 17 (Path A) made the free-text layer generalise across a *class* of
phrasings rather than fixed substrings — but it is still bounded by its
verb/noun vocabulary. `self_attack.py` measures this precisely:

- **Method 1 (vocabulary substitution): 5/5 miss.** "brought the experiment to
  a close as soon as the readings looked encouraging" (stopping verb outside
  `_OUTCOME_STOP_RE`), "no yardstick to measure our patients against"
  (comparison noun outside `_NO_COMPARISON_RE`), "a chap I met at a conference
  bar told me" (source phrasing outside `_INFORMAL_SOURCE_RE`).
- **Method 3 (cross-domain transfer): 2/4.** "our lone prototype" misses because
  "prototype" is not in the organism/person noun list; "nothing to benchmark
  our markets against" misses because "benchmark" is not in the comparison
  vocabulary. The same evidentiary shape in a non-biology register slips past.
- **Method 7 (paraphrase battery): 9/10.** "the one gentleman we enrolled"
  misses for the same reason.

I deliberately did **not** patch these by extending the lexicons. Two reasons,
both load-bearing: (1) adding the missing strings only defers the miss to the
next paraphrase — the round-1 mistake at a different level; (2) it provably
trades precision — broadening single-subject nouns to include "trial/run/
sample" makes the gate false-fire on a legitimate "single large trial of
4,000". The vocabulary boundary is doing real precision work. The principled
escape is the semantic scorer (future-work item #1), which collapses synonyms
to the same vector region; adding more tokens is a treadmill.

### 7.9 Independent stress test (`extra_probes.py`)

To check that the structural scoring isn't just passing the reviewer's
specific probes, I authored an independent probe set covering nine
evidentiary failure categories the reviewer's sets don't directly hit, plus
plain-language PASS cases. Iteration 17 added two Path-A categories (J, K),
taking it to 36 cases. Result at the same tuned threshold (0.80, unchanged):

| Category | Score |
|---|---|
| A. Correlation framed as causation | 3/3 |
| B. Effect-vs-n implausibility | 3/3 |
...
| J. Path-A generalisation: fresh weak paraphrases | 5/5 |
| K. Path-A false-positive guards (strong claims that look weak) | 5/5 |
| **Aggregate (PASS + FAIL only)** | **29/35 = 82.86%** |

PASS detection is robust across phrasing variation (**12/12**, including the
cat-K guards); the gate doesn't false-fire on plainly written strong claims, nor
on strong claims phrased to *resemble* weak ones ("not a single one was lost", an
expert "forum", a within-subject crossover with "no separate comparison arm").
Cat K is where iteration 17 found and fixed the bare-"forum" overfit. FAIL
detection generalises across most categories; the residual misses (E
population-mismatch, parts of D/F) are the semantic ceiling and motivate the
future-work backend (§8b item 1).

### 7.10 Self-attack battery (`self_attack.py`)

`self_attack.py` runs nine diagnostics from the round-3 reviewer's "how do I
attack my own system" list (the tenth, adversary-first authoring, is the
discipline the whole file follows). It is a diagnostic, not a pass/fail gate —
misses are findings. Summary of what it established:

| Method | Result | What it means |
|---|---|---|
| 1 Vocabulary substitution | 5/5 miss | free-text layer is vocabulary-bound (§7.8) |
| 2 Dict vs free-text | 3/3 catch | both layers catch single-subject; misleading `n` doesn't break it |
| 3 Cross-domain transfer | 2/4 | noun/verb lists are biology-register-bound |
| 4 Minimal perturbation | robust | margin is not single-word-fragile |
| 5 Null evidence dict | 5/5 catch | free-text layer still catches with `{}` — not dict-dependent |
| 6 PASS-case stripping | 5/5 PASS | no over-weighting of any single positive field |
| 7 Paraphrase battery | 9/10 | one noun-outside-vocab miss |
| 8 Compound vs single | monotonic | stacking weaknesses raises DAS (saturates at clip) |
| 9 Threshold sensitivity | clean | 0.80 is a real boundary, not a smear |

Two findings hardened the code (forum precision; the `_SCHEDULED_STOP_TOKENS`
schedule-vs-outcome stopping guard). The rest are the documented ceiling — see
§7.8 for why extending the lexicons would be overfitting, not a fix.
...
**Update (round 3): this is exactly what happened.** The round-3 reviewer sent
`heldout_datss_probes_v2.py`, authored to evade the token lists. I followed the
plan below: named the failure class (dict-encoded vs free-text-only weakness),
quantified it (3/8, the free-text-only cases at the base prior), and — crucially
for step 5 — did **not** add the probe phrasings to any list. Path A is
structure extraction over a *class* of phrasings, validated on independently
authored paraphrases (`path_a_generalization_check.py`, 10/10) and guarded
against over-firing (`extra_probes` cat K). The vocabulary ceiling that remains
(§7.8) is named, not hidden. The plan, for the next time:
...
1. **Semantic challenger backend** — wrap an embedding similarity scorer
   ...
   §7.8 vocabulary ceiling. **Path A (iteration 17) is the interim step
   toward this** — it extracts structure from free text with regexes, which
   generalises across a class of phrasings but not across the full semantic
   space; embeddings are what make "the one gentleman", "no yardstick", and
   cross-domain transfers collapse to the same region as their in-vocabulary
   equivalents.
...
| `datss/pool/structural.py` | Evidence-shape scorer: dict parsing + Path-A free-text extractors (it. 14/16/17) | 795 |
...
| `heldout_datss_probes_v2.py` | Round-3 reviewer's 8-case probe set (verbatim); 3/8 → 8/8 | 80 |
| `path_a_generalization_check.py` | Path-A generalisation + honest-ceiling check (10/10 + 3/3) | 110 |
| `self_attack.py` | Nine-method self-attack battery (§7.10) | 220 |
| `extra_probes.py` | Independent 36-case stress test, cats A–K | 380 |
...
adversarial corpus (92.59% as the honest out-of-distribution check), the
gate-direction note, and the YAML mirror test. Round 3 (iteration 17) added
Path A — free-text structure extraction that took the reviewer's v2 probes
from 3/8 to 8/8 without a threshold change — plus the self-attack battery and
the calibration work that made the docs state exactly what each scoring layer
does and where the vocabulary ceiling still bites.
